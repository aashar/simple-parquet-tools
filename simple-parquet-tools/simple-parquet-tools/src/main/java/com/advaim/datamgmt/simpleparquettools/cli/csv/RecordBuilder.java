...
package com.advaim.datamgmt.simpleparquettools.cli.csv;

import com.advaim.datamgmt.simpleparquettools.cli.util.RecordException;
    ...
    try {
      switch (schema.getType()) {
        ...
        case INT:
          if (string.isEmpty())
        	return null;
          if (schema.getLogicalType() != null && schema.getLogicalType().getName().equalsIgnoreCase("date") && schema.getProp("datetimeFormat") != null) {
        	DateTimeFormatter format = DateTimeFormatter.ofPattern(schema.getProp("datetimeFormat"));
        	return (int) LocalDate.parse(string, format).toEpochDay();
          }
